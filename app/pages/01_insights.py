...
@st.cache_data
def load_data():
    return pd.read_csv('data/processed/telco_engineered.csv')
...
contract_churn = df.groupby('Contract_Month-to-month')['Churn Value'].mean().reset_index()
...
with col4:
    ...
    fig4 = px.histogram(df, x='Monthly Charges', color=df['Churn Value'].map({0: 'No Churn', 1: 'Churn'}),
...
@st.cache_resource
def load_shap():
    shap_values = np.load('src/models/shap_values.npy')
...
st.markdown("<h3>Demographics vs Churn</h3>", unsafe_allow_html=True)
...
def load_churn_reasons():
    return pd.read_csv('data/processed/churn_reasons.csv')
...
st.plotly_chart(fig5, use_container_width=True)  # shap - last